				...
				if ( isset($_POST['install']) ) {
					switch ($_POST) {
						...
						default:
							# 开始安装
                      		//创建图片分类表
                      		$migs_db->query("CREATE TABLE IF NOT EXISTS `mi_category` (`id` varchar(20) NOT NULL,`cate_name` varchar(20) default NULL,`desc` varchar(100) default NULL,`orderby` int(2) NOT NULL COMMENT '排序码',PRIMARY KEY  (`id`)) DEFAULT CHARSET=utf8;");
                      		//插入默认分类
                      		$migs_db->query("INSERT INTO `mi_category` (`id`, `cate_name`, `desc`, `orderby`) VALUES ('default', '默认相册', '默认相册', 99);");
							// 创建图片存储表
                      		$migs_db->query("CREATE TABLE IF NOT EXISTS `mi_photos` (`uid` int(11) NOT NULL auto_increment,`author` bigint(20) default NULL,`name` varchar(64) default NULL,`title` varchar(64) character set utf8 default NULL,`description` longtext,`size` int(64) default NULL,`width` int(64) default NULL,`height` int(64) default NULL,`upload_time` datetime default NULL,`update_time` datetime default NULL,`storage` varchar(20) default NULL,`status` varchar(20) default NULL,`category` varchar(20) NOT NULL default 'default' COMMENT '图片分类',PRIMARY KEY  (`uid`),UNIQUE KEY `name` (`name`));");
							// 创建用户表
							$migs_db->query("CREATE TABLE IF NOT EXISTS `mi_users` (`uid` int(11) NOT NULL auto_increment,`name` varchar(20) default NULL,`pass` varchar(64) default NULL,`email` text,`authkey` varchar(64) default NULL,`authtime` datetime default NULL,PRIMARY KEY  (`uid`),UNIQUE KEY `name` (`name`));");	
                      		// 创建选项表
							$migs_db->query("CREATE TABLE IF NOT EXISTS `mi_options` (`uid` int(11) NOT NULL auto_increment,`option_name` varchar(64) default NULL,`option_value` longtext,PRIMARY KEY  (`uid`),UNIQUE KEY `option_name` (`option_name`));");
