...
<?php $category_info = $migs_db->get_results("SELECT * FROM mi_category WHERE id='$photo->category' LIMIT 0,1");
	 $current = $category_info[0];?>
<span id="category-<?php echo $photo->uid;?>" class="editable" title="<?php echo $current->desc; ?>"><?php echo $current->cate_name; ?></span>
<?php $category_info = $migs_db->get_results("SELECT id,cate_name FROM mi_category ORDER BY orderby ASC");?>
<select id="editcategory-<?php echo $photo->uid;?>" name="category" class="edititle" onblur="update_category('<?php echo $photo->uid;?>');">
<?php foreach($category_info as $single){?>
	<option value="<?php echo $single->id; ?>"<?php if($single->id == $photo->category){?> selected="selected"<?php }?>><?php echo $single->cate_name; ?></option>	
<?php }?>
</select>
                                </li>
